...
// local functions
int (*extend_shortest_path(int (*)[MAX_VERTICES], int (*)[MAX_VERTICES], int vernum))[MAX_VERTICES];
...
int (*extend_shortest_path(int (*l)[MAX_VERTICES], int (*laux)[MAX_VERTICES], int vernum))[MAX_VERTICES]
{
  ...
  for (int i = vernum-1; i >= 0; i--) {
	...
	if (l[i][k] != INT_MAX && laux[k][j] != INT_MAX) {
	  if (l[i][k] + laux[k][j] < r[i][j]) {
	    r[i][j] = l[i][k] + laux[k][j];
	  }
	}
      }
    }
...
int (*slow_all_pairs_shortest_paths(int vernum))[MAX_VERTICES]
{
  ...
  int m = 1;
  while (m < vernum-1) {
    l = extend_shortest_path(l, l, vernum);
    m *= 2;
  }
  return l;
}
